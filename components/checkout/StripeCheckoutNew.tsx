  ...
  useEffect(() => {
    if (elements) {
      ...
      timeoutId = setTimeout(() => {
        console.log('Payment Element timeout - marking as ready anyway');
        setIsReady(true);
      }, 3000); // 3 seconds
      ...
      if (paymentElement) {
        ...
        setTimeout(() => {
          ...
          if (element) {
            ...
            console.log('Payment Element not found, but proceeding anyway');
            setIsReady(true);
          }
        }, 1000);
      }
...
export default function StripeCheckout({ clientSecret, amount, currency, onSuccess, onError }: StripeCheckoutProps) {
  console.log('StripeCheckout props:', { clientSecret: clientSecret ? 'provided' : 'missing', amount, currency });
