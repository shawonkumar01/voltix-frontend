...
import { z } from "zod";
import { Eye, EyeOff, Zap, ArrowRight, Loader2, Globe, Check } from "lucide-react";
...
export default function RegisterPage() {
    ...
    return (
        <div className="min-h-screen bg-[#080808] flex items-center justify-center relative overflow-hidden px-4 py-12">
            {/* Ambient background */}
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-[-10%] left-[-10%] w-[600px] h-[600px] bg-cyan-500/7 rounded-full blur-[120px]" />
                ...
                <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-8 backdrop-blur-2xl shadow-[0_0_80px_rgba(0,0,0,0.6)]">
                    ...
                    >
                        ...
                        <p className="text-sm text-white/40">
                            Join Voltix and shop the future
                        </p>
                    </motion.div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                        ...
                        >
                            <div>
                                ...
                                />
                                {errors.firstName && (
                                    <p className="text-xs text-red-400/80 mt-1.5 ml-1">{errors.firstName.message}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-white/40 uppercase tracking-widest mb-2">
                                ...
                                />
                                {errors.lastName && (
                                    <p className="text-xs text-red-400/80 mt-1.5 ml-1">{errors.lastName.message}</p>
                                )}
                            </div>
                        </motion.div>
                                ...
                                >
                                    {passwordChecks.map(({ label, test }) => (
                                        <div key={label} className="flex items-center gap-2">
                                            <div
                                                className={`w-3.5 h-3.5 rounded-full flex items-center justify-center transition-all duration-200 ${test(password)
                                                        ? "bg-cyan-400/20 border border-cyan-400/40"
                                                        : "bg-white/5 border border-white/10"
                                                    }`}
                                            >
                                                {test(password) && <Check className="w-2 h-2 text-cyan-400" />}
                                            </div>
                                            <span
                                                className={`text-[11px] transition-colors duration-200 ${test(password) ? "text-cyan-400/70" : "text-white/25"
                                                    }`}
                                            >
                                                {label}
                                            </span>
                        ...
                        >
                            ...
                            {errors.confirmPassword && (
                                <p className="text-xs text-red-400/80 mt-1.5 ml-1">{errors.confirmPassword.message}</p>
                            )}
                        </motion.div>
